...
export default defineComponent({
  ...
  setup() {
    ...
    const getPriceList = async () => {
      console.log(router);
      await axios
        .post("http://127.0.0.1:8000/api/priceList", {
    ...
    onMounted(() => {
      ...
      console.log(`BidChannel.${router.currentRoute.value.params.id}`);

      //   sessionStorage.setItem("user", router.currentRoute.value.params.user);

      window.Echo.channel(
        `BidChannel.${router.currentRoute.value.params.id}`
      ).listen(".BidPrice", function (e) {
